...
export default class SmsController {
    ...
    @ApiOperation({summary: 'Get all sms sent by an application', description: 'This endpoint is protected and returns all the SMS a user has sent using this application'})
    @ApiResponse({
        type: smsDto,
        status: 200,
        description: 'Success'
    ...
    @UseGuards(JwtAuthGuard)
    @Post('sendsms/:application_id') 
    async sendsms(@Body() sms: smsDto, @Param('application_id') sms_id: string) {
        return this.Smsservice.sendSms(sms, sms_id);
    }

    @ApiOperation({ summary: 'Delete an SMS', description: 'An authenticated user can delete an SMS by making a request to this endpoint.' })
