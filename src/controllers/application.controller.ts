...
export class ApplicationController {
  ...
  @ApiOperation({
    summary: "Get all created applications",
    description:
      "List of all the apps created by a specific user(creator). This route is protected only authenticated users can access it",
  })
  @ApiResponse({
    type: ApplicationDto,
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      "This route is protected, only authenticated users can create an application",
  })
  @ApiResponse({
    type: ApplicationDto,
    status: 201,
  ...
  async createApplication(
    @Body() appDetails: ApplicationDto,
  ...
  @ApiOperation({
    summary: "Update an app status",
    description:
      "Updata an app status. This route is also protected, so a user must be authenticated to make changes to an application",
  })
  @Patch(":id/status")
  @UseGuards(JwtAuthGuard)
  updataAppStatus(
  ...
  @ApiOperation({
    ...
    description:
      "An authenticated user can delete an application by making a request to this endpoint.",
  })
  @UseGuards(JwtAuthGuard)
  @Delete(":id")
  ...
  @Patch(":id/regeneate_app_token")
  regenerateAppToken(@Param("id") _id: string) {
    return this.appService.regenerateAppToken(_id);
  }
}


// 4a194876-a689-4d79-9baa-8dcec2de16c1

// 425e9767-843d-4cd9-bc1e-d31e404c1733