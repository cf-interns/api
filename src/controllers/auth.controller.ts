...
export class AuthController {
  ...
  @ApiOperation({
    summary: "Get a new Token upon acess_token expiration",
    description:
      "Replace acess-token with the new refresh token in the browser cookie to prove authorization for subsequent requests to the api",
  })
  @ApiResponse({
    // type: ,
    status: 200,
  ...
  @Post("sign_up")
  async register(
    @Body() signUpUser: RegisterDataDto,
  ...
  @ApiOperation({
    ...
    description: "Validate user and login if successful",
  })
  @ApiResponse({
    type: LoginDto,
    status: 200,
  ...
  async login(@Req() req: RequestObjectWithUser, @Body() loginData: LoginDto) {
    ...
    const { cookie: refreshTokenCookie, refresh_token: refresh_token } =
  ...
  @ApiOperation({
    ...
    description: "Remove user authentication token in the browser cookie",
  })
  @ApiResponse({
    status: 200,
    // description: ''
